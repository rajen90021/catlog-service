...
export class ProductController {
  ...
  create = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { name, description, priceConfiguration, attributes, tenantId, categoryId, isPublish } =
        req.body;
      ...
      if (imageFile) {
        ...
        const uploadResponse = await this.cloudinaryStorage.upload(fileData, 'products');
        ...
        JSON.stringify({
          ...
          data: {
            ...
            priceConfiguration: mapToObject(
              response.priceConfiguration as unknown as Map<string, any>
            ),
          },
        })
      );
    ...
    try {
      ...
      if (req.files?.image) {
        ...
        if (product.image && product.image.includes('cloudinary')) {
          await this.cloudinaryStorage.delete(
